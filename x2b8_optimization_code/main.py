...
def load_data():
    DATA_DIR = '../../CarpeDiem Extubation Prediction Sam Alec/data/edw_redacted'
...
def run_models_and_collect_results(model_types, imputation_methods, hyperparams, print_args:
    ...
    for model_type in model_types:
        ...
        for method in imputation_methods:
            ...
            metric_results, y_true, preds_proba, X_train, X_val, X_test, model, model_type = ModelTrainer.run_model(train_data, val_data, test_data, model_type, method, hyperparams, print_args)
...
def plot_all_metrics(results_df, prediction_data):
    ...
    plt.savefig('results/auroc_all.png')

    plt.show()

def main():
    model_types = ['RNN','LSTM','LightGBM','LogisticRegression','XGBoost','RandomForest']
    imputation_methods = ['binning','mean','raw']
    
    # model_types = ['LightGBM','LogisticRegression','XGBoost']
    # imputation_methods = ['binning','raw']
